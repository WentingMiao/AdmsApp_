...
def account_register(request):
    ...
    try:
        ...
        if not email or not username or not password1 or not password2:
            ...
            render(request, "account/register.html", {"error": err})
...
def members_management(request):
    ...
    if content :
        queryset = queryset.filter(Q(name__icontains=content)|Q(department__icontains=content)|Q(position__icontains=content)|Q(uni__icontains=content))
